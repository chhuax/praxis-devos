...
- 提案获批并进入实现：先检查当前分支；若用户已经在该 change 的专用实现分支，可直接复用，否则再创建 Git 分支
- 合并完成且行为已落地：再归档提案
...
1. `npx praxis-devos openspec validate <change-id> --strict --no-interactive`
2. 检查当前分支；若不是该 change 的专用实现分支，则创建实现分支
3. 开始编码与验证
4. 发起 PR，并关联 `change-id`
5. 合并后再执行 `npx praxis-devos openspec archive <change-id> --yes`

### 当前分支复用规则

- 如果当前分支已经是与已批准提案对应的专用实现分支，SHOULD 直接复用，不要强制再切新分支。
- 如果当前分支是 `main`、`master`、`develop`、`release/*`，或明显承载其他任务，MUST 先切到新的实现分支。
- 如果当前分支名称与 `change-id` 无关，但变更范围、提交历史和工作区都明确只服务当前提案，可在确认后继续复用。
- 如果无法判断当前分支是否安全可复用，默认按更安全方式新建实现分支。
