...
当前方向已经明确：Praxis 的文档契约 Phase 1 已经围绕 `contracts/surfaces.yaml` 和 `docs/codemaps/` 落了一版轻量实现。问题不在于“这套实现是否存在”，而在于它当前仍以脚本式生成为主路径。Phase 2 的目标是把这套能力演进成 AI-first 编排，同时和现有 Phase 1 保持兼容。
...
- 不在本次 change 中执行 `contracts/surfaces.yaml` -> `docs/surfaces.yaml` 的路径迁移
...
- 必要文件是否存在
- 路径引用是否有效
- `contracts/surfaces.yaml` 是否合法
- codemap 是否残留占位符或空托管区块
- 多模块检测成立时，`module-map.md` 和 `modules/<artifactId>.md` 是否齐全

路径优先级：

- 在本阶段，`contracts/surfaces.yaml` 是 canonical path
- 如果同时出现 `contracts/surfaces.yaml` 和 `docs/surfaces.yaml`，校验应报告冲突
- `docs/surfaces.yaml` 不作为本阶段有效输出路径
...
- 使用同一个 canonical path：`contracts/surfaces.yaml`
...
当前阶段只允许 `contracts/surfaces.yaml` 作为 canonical path。

- `docs/surfaces.yaml` 不是有效输出目标
- 如果 writeback contract 试图写入 `docs/surfaces.yaml`，validation 必须在 repository writeback 前拒绝该结果
- 如果仓库中同时存在 `contracts/surfaces.yaml` 与 `docs/surfaces.yaml`，`docs check` 必须报告冲突
...
- 将已有 `contracts/surfaces.yaml` 自动移动到 `docs/surfaces.yaml`
- 将已有 `contracts/surfaces.yaml` 自动重命名为其他路径
