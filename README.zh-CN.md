...
- **通用 Skills**（数据库、安全、异常处理、Redis、Git 等通用最佳实践）
...
│   ├── changes/                   # 活跃变更提案
│   └── templates/                 # 提案/任务模板
├── skills/                        # 通用 Skills（所有栈共享）
│   ├── openspec-workflow/         # OpenSpec 工作流
│   ├── git-workflow/              # Git 分支、提交、合并
│   ├── database-guidelines/       # 数据库设计（通用）
│   ├── error-handling/            # 异常处理（通用）
│   ├── security/                  # 安全编码（OWASP）
│   └── redis-guidelines/          # Redis 最佳实践（通用）
└── stacks/                        # 可插拔技术栈
...
| database-guidelines | 通用数据库设计、SQL 安全、索引原则 |
| error-handling | 通用异常处理、结构化错误响应（RFC 7807） |
| security | 安全编码规范（OWASP Top 10） |
| redis-guidelines | Redis 最佳实践、缓存模式 |

### 栈专属 Skills（按技术栈加载）

技术栈可定义自己的专属 skills，扩展通用 skills。例如 `yonbip-java` 栈提供：
- `yonbip-database` — 多租户字段规范（扩展 database-guidelines）
- `yonbip-error-handling` — 异常码体系（扩展 error-handling）
- `yonbip-security` — 安全扫描工具链（扩展 security）
- `yonbip-redis` — 多租户 Key 设计（扩展 redis-guidelines）
