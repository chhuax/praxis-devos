...
const ECC_HOOKS_ADAPTER_DIR = 'ecc-hooks';
const ECC_HOOKS_ADAPTER_MANIFEST = 'manifest.json';
...
const resolveExecutableFromDir = (rootDir, executable) => {
  const names = process.platform === 'win32'
    ? [`${executable}.cmd`, `${executable}.bat`, `${executable}.exe`, executable]
    : [executable];
  ...
  for (const name of names) {
    ...
    const binPath = path.join(rootDir, 'bin', name);
...
const resolveEccRuntime = (projectDir) => {
  for (const envName of ECC_RUNTIME_ENV_VARS) {
    ...
    const resolvedPath = path.isAbsolute(rawValue) ? rawValue : path.resolve(projectDir, rawValue);
    const stats = pathStats(resolvedPath);
    ...
    if (!stats.exists) {
      return {
        ...
        path: resolvedPath,
        command: null,
        detail: `${envName} is set to ${resolvedPath}, but that path does not exist.`,
      };
    }
    ...
    return {
      ...
      command,
      detail: stats.isDirectory
        ? `ECC runtime root is bound via ${envName} at ${resolvedPath}${command ? ` (detected ${ECC_EXECUTABLE} at ${command})` : ''}`
        : `ECC runtime is bound via ${envName} at ${resolvedPath}`,
    };
  }
  ...
  if (fs.existsSync(localPath)) {
    return {
      ...
      path: localPath,
      command: localPath,
      detail: `ECC runtime CLI is available via ${localPath}`,
    };
  }

  const globalPath = findCommandPath(ECC_EXECUTABLE);
  if (globalPath) {
    return {
      status: 'ok',
      source: 'path',
      path: globalPath,
      command: globalPath,
      detail: `ECC runtime CLI is available on PATH via ${globalPath}`,
    };
  }
...
const projectPaths = (projectDir) => {
  ...
  return {
    ...
    praxisEccHooksAdapterManifestPath: path.join(praxisDir, 'adapters', ECC_HOOKS_ADAPTER_DIR, ECC_HOOKS_ADAPTER_MANIFEST),
...
const resolveEccBinding = ({ projectDir, foundationName = null }) => {
  const runtimeBase = resolveFoundationRuntimeBase(foundationName);
  if (runtimeBase !== 'ecc') {
    return {
      required: false,
      runtimeBase,
      ...
      path: null,
      command: null,
      detail: 'ECC runtime binding is not required because no ECC runtime base preset is applied.',
    };
  }

  const runtime = resolveEccRuntime(projectDir);
  return {
    required: true,
    runtimeBase,
    state: runtime.status === 'ok' ? 'bound' : 'unbound',
...
const renderEccHooksAdapterReadme = ({ eccBinding, descriptorRelativePath }) => {
  const lines = [
    '# ECC Hooks Adapter',
    '',
    'This directory is a generated Praxis adapter layer for the minimal ECC hooks bridge.',
    '',
    `- runtime required: ${eccBinding.required ? 'yes' : 'no'}`,
    `- binding state: ${eccBinding.state}`,
    `- binding source: ${eccBinding.source}`,
...
const resolveEccHooksAdapterStatus = ({ projectDir, foundationName = null }) => {
  const paths = projectPaths(projectDir);
  const eccBinding = resolveEccBinding({
    projectDir,
    foundationName,
  });
  const manifest = readJson(paths.praxisEccHooksAdapterManifestPath);
  const hasManifest = Boolean(manifest);
  const hasDescriptor = fs.existsSync(paths.praxisEccHooksAdapterDescriptorPath);

  if (!eccBinding.required) {
    return {
      status: 'ok',
      detail: 'ECC hooks adapter is not required because no ECC runtime base preset is applied.',
    };
  }

  if (eccBinding.status === 'ok' && hasDescriptor) {
    return {
      status: 'ok',
      detail: `ECC hooks descriptor is ready at .praxis/adapters/${ECC_HOOKS_ADAPTER_DIR}/hooks/${path.basename(paths.praxisEccHooksAdapterDescriptorPath)}`,
    };
  }
  ...
  if (eccBinding.status === 'ok') {
    return {
      status: 'warning',
      detail: 'ECC runtime is bound, but the hooks descriptor is missing. Re-run `npx praxis-devos sync`.',
    };
  }
  ...
  if (hasManifest) {
    return {
      status: 'warning',
      detail: `ECC hooks adapter placeholder is present, but ECC is not bound yet. Run \`${ECC_BIND_COMMAND}\`.`,
    };
  }

  return {
    status: 'missing',
    detail: 'ECC hooks adapter is missing. Re-run `npx praxis-devos sync` after applying the ECC runtime base.',
  };
};
...
const buildProjectDependencies = ({ projectDir, selectedFoundation, existingDependencies = {} }) => {
  ...
  const eccBinding = resolveEccBinding({
    projectDir,
    foundationName: selectedFoundation,
  });

  return {
    ...existingDependencies,
    openspec: {
      ...(existingDependencies.openspec || {}),
      required: true,
      type: 'cli',
    },
    superpowers: {
      ...(existingDependencies.superpowers || {}),
      required: true,
      type: 'agent-runtime',
...
const ensurePraxisManifest = ({ projectDir, stackName, agents, migratedFrom }) => {
  ...
  const nextManifest = {
    ...
    dependencies: buildProjectDependencies({
      projectDir,
      selectedFoundation,
      existingDependencies: manifest.dependencies || {},
    }),
...
const syncEccHooksAdapter = ({ projectDir, foundationName, log }) => {
  const paths = projectPaths(projectDir);
  const eccBinding = resolveEccBinding({
    projectDir,
    foundationName,
  ...
  if (eccBinding.status === 'ok' && eccBinding.command) {
    ensureDir(paths.praxisEccHooksAdapterHooksDir);
    writeJson(
  ...
  removePathIfExists(paths.praxisEccHooksAdapterHooksDir);
  log(`✓ .praxis/adapters/${ECC_HOOKS_ADAPTER_DIR}/ synced (placeholder; ${eccBinding.source})`);
};
...
const renderFoundationReadme = (definition, eccBinding = null) => {
  ...
  const bindingState = definition.runtimeBase === 'ecc' && eccBinding
    ? eccBinding.state
    : 'not-required';
  const bindingSource = definition.runtimeBase === 'ecc' && eccBinding
    ? eccBinding.source
    : 'not-required';
  const bindingNote = definition.runtimeBase === 'ecc' && eccBinding
    ? eccBinding.detail
    : 'ECC runtime binding is not required for this foundation.';
...
- ECC binding state: \`${bindingState}\`
- ECC binding source: \`${bindingSource}\`
...
- Current ECC binding note: ${bindingNote}
...
export const statusProject = ({ projectDir, agents = SUPPORTED_AGENTS }) => {
  ...
  const manifest = readJson(paths.manifestPath);
  const foundationManifest = readJson(paths.praxisFoundationManifestPath);
  const foundationName = manifest?.selectedFoundation || foundationManifest?.foundation || null;
  const eccBinding = resolveEccBinding({
    projectDir,
    foundationName,
  });
...
export const doctorProject = ({ projectDir, agents = SUPPORTED_AGENTS, strict = false }) => {
  ...
  const results = [];
  const manifest = readJson(projectPaths(projectDir).manifestPath) || {};
  const foundationManifest = readJson(projectPaths(projectDir).praxisFoundationManifestPath) || {};
  const foundationName = manifest.selectedFoundation || foundationManifest.foundation || null;
  const eccBinding = resolveEccBinding({
    projectDir,
    foundationName,
